...
class Explainer:
    ...
    def get_raw_attn_explanations(self, input_ids, attention_mask, starting_layer=0, last_layer=6):
        output = self.model(
        ...
        attn_weights = np.array([item.cpu().detach().numpy() for item in output.attentions[starting_layer: last_layer]])
